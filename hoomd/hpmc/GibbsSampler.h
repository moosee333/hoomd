...
//NOTE: Do I have to incorporate the Poisson distribution explicitly? I think so... for now I'm just trying to insert the same number of particles every time
//NOTE: The densities I'm currently specifying are wrong; I'm doing it in terms of box volume, but it should actually be in terms of free volume... right?
 */
namespace hpmc
{
template< class Shape > class GibbsSampler : public ExternalFieldMono<Shape>
    ...
    public:
          ...
          */
        GibbsSampler(std::shared_ptr<SystemDefinition> sysdef, std::shared_ptr<IntegratorHPMCMono<Shape>> mc, std::vector<unsigned int> type_indices, std::vector<Scalar> type_densities) : ExternalFieldMono<Shape>(sysdef), m_mc(mc), m_type_indices(type_indices), m_type_densities(type_densities), m_num_species(type_densities.size())
            {
            ...
            for(unsigned int i = 0; i < m_num_species.size(); i++)
                ...
                int retval = posix_memalign((void**)&temp_aabb, 32, m_type_samples[i]*sizeof(detail::AABB));
...
Simplest solution might just be to provide an array of types corresponding to the positions array
                ...
                for(unsigned int j = 0; j < m_type_indices.size(); j++)
                    ...
                    if(m_type_indices[j] == type_indices[i])
                        ...
                        if (new_sample_size > m_type_samples[j])
                            {
                        // NOTE: This isn't quite so simple. We can't get rid of the old AABBs, we have to somehow leave them in place. Which in practice will probably just involved copying them in
                                if (m_aabbs[j] != NULL)
                                    free(m_aabbs[j]);
    ...
    protected:
        ...
        std::pair<std::vector<vec3<Scalar>>, std::vector<quat<Scalar>>> generateRandomConfiguration(unsigned int type_index_i, unsigned int timestep)
            ...
            for (unsigned int i = 0; i < m_type_samples[type_index_i]; i++)
                {
                overlap = false;
