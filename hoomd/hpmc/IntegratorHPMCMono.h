            ...
            for (unsigned int cur_particle = 0; cur_particle < m_pdata->getN(); cur_particle++)
                ...
                for (unsigned int cur_particle = 0; cur_particle < m_pdata->getN(); cur_particle++)
                    ...
                    if (move_type_translate)
                        ...
                        move_translate_sphere(shape_i.quat_l, shape_i.quat_r, rng_i, h_d.data[typ_i], ndim, sphere);
                        }
                    else
                        {
                        if (h_a.data[typ_i] == 0.0)
                        ...
                        move_rotate_sphere(shape_i.quat_l, shape_i.quat_r, rng_i, h_a.data[typ_i], ndim);
                        }
                    ...
                    if (!overlap)
                        ...
                        // update spherical coordinates of particle
...
unsigned int IntegratorHPMCMono<Shape>::countOverlaps(unsigned int timestep, bool early_exit)
    ...
    if (m_pdata->getBoundaryConditions() == ParticleData::periodic)
        ...
        for (unsigned int i = 0; i < m_pdata->getN(); i++)
            ...
            // check if a particle intersects itself
            if (test_self_overlap_sphere(shape_i, sphere))
                {
                overlap_count++;
                if (early_exit)
        ...
        throw std::runtime_error("Unknown boundary conditions in countOverlaps().\n");
