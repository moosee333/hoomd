...
class sphere_resize(_updater):
    R""" Rescale the system sphere radius.

    Args:
        R (:py:mod:`hoomd.variant`): sphere radius as a function of time (in distance units)
        period (int): The sphere size will be updated every *period* time steps.
        phase (int): When -1, start on the current time step. When >= 0, execute on steps where *(step + phase) % period == 0*.

    Every *period* time steps, the system sphere radius is updated to values given by
    ...
    Examples::
        ...
        update.sphere_resize(R = hoomd.variant.linear_interp([(0, 20), (1e6, 50)]))
        box_resize = update.box_resize(R = hoomd.variant.linear_interp([(0, 20), (1e6, 50)]), period = 10)
    """
    ...
    def __init__(self, R, period = 1, phase=0, scale_particles=True):
        ...
        # create the c++ mirror class
        self.cpp_updater = _hoomd.SphereResizeUpdater(hoomd.context.current.system_definition, R.cpp_variant);
