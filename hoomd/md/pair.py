...
class yukawa(pair):
    ...
    .. math::
        ...
        \begin{eqnarray*}
         V_{\mathrm{yukawa}}(r)  = & \varepsilon q_i q_j \frac{ \exp \left[ -\kappa \left(r-\frac{D_i+D_j}/2\right) \right] }{\left(1+\frac12 \kappa*D_i\right)\left(1+\frac12 \kappa*D_j\right)}r} & r < r_{\mathrm{cut}} \\
                            = & 0 & r \ge r_{\mathrm{cut}} \\
        \end{eqnarray*}

    Here, :math:`D_i` and :math:`D_j` are the particle diameters. They are only used if **use_diameter** is set,
    otherwise are taken to be zero. With particle diameters, :py:class`yukawa` can be used to implement Debye-Hueckel
    screened interactions. Analogously, :math:`q_i` and :math:`q_j` are the particle charges, which
    are only used when **use_charge=True**, otherwise they are set equal to one.
    ...
    - **use_diameter** - True if we should use particle diameters (default: False)
        .. versionadded:: 2.2
    - **use_charge** - True if we should use particle charges (default: False)
        .. versionadded:: 2.2
    - :math:`r_{\mathrm{cut}}` - *r_cut* (in distance units)
      - *optional*: defaults to the global r_cut specified in the pair command
    - :math:`r_{\mathrm{on}}`- *r_on* (in distance units)
    ...
    Example::
        ...
        yukawa.pair_coeff.set('A', 'A', epsilon=1.0, kappa=1.0,use_diameter=True, use_charge=True)
    ...
    def __init__(self, r_cut, nlist, name=None):
        ...
        # setup the coefficent options
        self.required_coeffs = ['epsilon', 'kappa','use_diameter','use_charge'];
    ...
    def process_coeff(self, coeff):
        ...
        return _hoomd.make_scalar4(epsilon, kappa, int(use_diameter), int(use_charge));
